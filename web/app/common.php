...
/**
 * 根据code获取省市 1 市 2 省 3学校
 */
function getAreaByCode($code, $flag = 1)
{
    if ($flag == 1) {
        return D('Area')->getCityNameByCode($code);
    }
    switch ($flag) {
        case 1:
            return D('Area')->getCityNameByCode($code);
            break;
        case 2:
            return D('Area')->getProNameByCode($code);
            break;
        case 3:
            $res = D('School')->getById($code);
            return $res['name'];
            break;
    }
}

		...
		<div class="list-search">
			<form action="" class="form-inline" method="get" >
			  <div class="form-group">
			    <label for="select-pos">板块：</label>
			    <select name="section" class="form-control" style="width:100px" id="select-sec">
				  <option value="-1">全部</option>
				  <?php if(is_array($sections) || $sections instanceof \think\Collection): if( count($sections)==0 ) : echo "" ;else: foreach($sections as $k=>$model): ?>
				  <option value="<?php echo $k; ?>"><?php echo $model['title']; ?></option>
				  <?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			  </div>
			  <div class="form-group">
			    <label for="select-status">标签：</label>
				<input name="title" class="form-control" style="width:200px" value="<?php echo isset($cond['title']) ? $cond['title'] :  ''; ?>" />
			  </div>
			  <button type="submit" class="btn btn-default">查询</button>
			  <script>
			  	$('#select-sec').val('<?php echo isset($cond['section']) ? $cond['section'] :  -1; ?>');
			  </script>
			</form>
		</div>
		<div class="list-btns">
			<?php if(authority('Tag_add')): ?>
			<button type="button" class="btn btn-success" id="addItem">新建</button>
			<?php endif; if(authority('Tag_del')): ?>
		  ...
		  <tbody>
			<?php if(is_array($list) || $list instanceof \think\Collection): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$model): ?>
				<tr>
					...
					<td><?php echo $sections[$model['section']]['title']; ?></td>
					<td><?php echo $model['title']; ?></td>
					<td><?php if(authority('Tag_edit')): ?><a href="__PRO_PATH__/Tag/edit?id=<?php echo $model['id']; ?>">编辑</a><?php endif; ?></td>
				</tr>
			<?php endforeach; endif; else: echo "" ;endif; ?>
		  </tbody>
		</table>
...
<script>
	$('#lab-nav6').addClass('open active');
	$('#lab-nav6-sub1').addClass('active');
	popupList({

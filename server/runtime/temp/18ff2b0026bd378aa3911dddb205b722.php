		...
		<div class="list-search">
			<form action="" class="form-inline" method="get" >
			  ...
			  <div class="form-group">
			    ...
			    <input name="title" class="form-control" style="width:200px" id="form-title" value="<?php echo isset($cond['title']) ? $cond['title'] : ''; ?>" />
			  </div>
			  <button type="submit" class="btn btn-default">查询</button>
			  <script>
			  	$('#form-tag').val('<?php echo isset($cond['tagid']) ? $cond['tagid'] :  -1; ?>');
			  </script>
			</form>
		</div>
		<div class="list-btns">
			<?php if(authority('ResearchArea_add')): ?>
			<button type="button" class="btn btn-success" id="addItem">新建</button>
			<?php endif; if(authority('ResearchArea_del')): ?>
		  ...
		  <tbody>
			<?php if(is_array($list) || $list instanceof \think\Collection): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$model): ?>
				<tr>
					...
					<td>
						<?php if(authority('ResearchArea_top')): ?>
						...
						<?php endif; if(authority('ResearchArea_recomm')): if($model['isrecommend']): ?>
						...
						<?php endif; endif; if(authority('ResearchArea_edit')): ?>
						<a href="__PRO_PATH__/ResearchArea/edit?id=<?php echo $model['id']; ?>">编辑</a>
						<?php endif; ?>
					</td>
				</tr>
			<?php endforeach; endif; else: echo "" ;endif; ?>
...
<script>
	...
	popupList({
		create:'__PRO_PATH__/ResearchArea/create',
		remove:'__PRO_PATH__/ResearchArea/remove'
	});
	function doPublish(id){
		$.get('__PRO_PATH__/ResearchArea/dopublish', {id:id}, function(res){
			if(res.code == 1){
				window.location.reload();
			}else{
				popWarn('发布失败');
	...
	function doRecommend(id){
		popupRecomm('__PRO_PATH__/ResearchArea/dorecomm', id);
	}
	function cancelRecomm(id){
		$.get('__PRO_PATH__/ResearchArea/cancelrecomm', {conid:id}, function(res){
			if(res.code == 1){
				window.location.reload();
			}else{
				popWarn(ret.msg);

    ...
    <div class="padding-md">
        ...
        <div class="list-search">
            <form action="" class="form-inline" method="get" >
                ...
                <div class="form-group">
                    <label for="select-research">研究方向：</label>
                    <select name="research_id" class="form-control" style="width:100px" id="select-research">
        ...
        <table class="table table-hover" style="background:#fff;">
            ...
            <tr>
                ...
                <td><?php echo $model['research_title']; ?></td>
                <td><?php echo $model['title']; ?></td>
                <td><?php echo $model['digest']; ?></td>
                <td><img src="<?php echo $model['img']; ?>" alt="" style="width:160px; height:50px;" /></td>
                <td><?php if($model['publishtime']): ?><?php echo date("Y-m-d H:m",$model['publishtime'] ); endif; ?></td>
                <td><?php echo $model['count_view']; ?></td>
                <td>
                    <?php if(authority('Result_top')): ?>
                    <a href="javascript:;" onclick="doTop(<?php echo $model['id']; ?>)">置顶</a>
                    <?php endif; if(authority('Result_recomm')): if($model['isrecommend']): ?>
                    ...
                    <?php endif; endif; if(!$model['ispublish']): ?>
                    <a href="javascript:;" onclick="doPublish(<?php echo $model['id']; ?>)">发布</a>
                    <?php endif; if(authority('Result_edit')): ?>
                    <a href="__PRO_PATH__/Result/edit?id=<?php echo $model['id']; ?>">编辑</a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
...
<script>
    ...
    popupList({
        create:'__PRO_PATH__/Result/create',
        remove:'__PRO_PATH__/Result/remove'
    });
    function doPublish(id){
        $.get('__PRO_PATH__/Result/dopublish', {id:id}, function(res){
            if(res.code == 1){
                window.location.reload();
            }else{
                popWarn('发布失败');
    ...
    function doRecommend(id){
        popupRecomm('__PRO_PATH__/Result/dorecomm', id);
    }
    function cancelRecomm(id){
        $.get('__PRO_PATH__/Result/cancelrecomm', {conid:id}, function(res){
            if(res.code == 1){
                window.location.reload();
            }else{
                popWarn(ret.msg);

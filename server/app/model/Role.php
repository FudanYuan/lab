...
class Role extends Model{
 	protected $table = 'lab_role_admin';
 	protected $pk = 'id';
 	...
 	public function getById($roleid){
 		...
 		if(!empty($res)){
 			$actions = Db::table('lab_role_action_admin')->where(['roleid' => $roleid, 'status' => 1])->column('actionid');
 			$res['actions'] = $actions;
 		}
 		return $res;
 	}
 	...
 	public function addRoleAction($roleid, $actionids){
 		...
 		return Db::table('lab_role_action_admin')->insertAll($data);
 	}
 	/**
 	 * 获取角色权限列表
 	 */
 	public function getRoleActions($roleid){
 		return Db::table('lab_role_action_admin')->where(['roleid' => $roleid, 'status' => 1])->column('actionid');
 	}
 	/**
 	 * 删除角色权限
 	 * @param unknown $roleid
 	...
 	public function removeRoleActions($roleid, $actionids){
 		return Db::table('lab_role_action_admin')->where(['roleid' => $roleid, 'actionid' => ['in', $actionids]])->delete();
 	}
 	/**
 	 * 根据角色获取操作列表
 	 */
 	public function getActionsByRoleId($roleid){
 		if(!$roleid) return [];
 		$res = [];
 		$actions = Db::table('lab_role_action_admin')->alias('a')->field('b.id,b.name,b.tag,b.pid,b.pids,level')
 			->where(['a.roleid' => $roleid, 'a.status' => 1])->join('lab_action_admin b', 'a.actionid=b.id', 'LEFT')
 			->column('tag');
 		return $actions;
 	}
 }
